---
layout: default
title: ポートフォリオ理論
nav_order: 2
---

# ポートフォリオ理論 (Portfolio Theory)

複数の資産を組み合わせることで、リスクを抑えつつリターンを最大化するための理論。

## 1. 現代ポートフォリオ理論 (MPT: Modern Portfolio Theory)
ハリー・マーコウィッツによって提唱された。
- **分散投資**: 相関の低い資産を組み合わせることで、リターンを維持したままポートフォリオ全体のリスク（標準偏差）を低減できる。
- **平均・分散アプローチ**: リターン（期待値）とリスク（分散）の2軸で投資対象を評価する。

## 2. 効率的フロンティア (Efficient Frontier)
あるリスクに対して最大のリターン、またはあるリターンに対して最小のリスクを実現するポートフォリオの集合。
- 投資家はこのフロンティア線上にあるポートフォリオを選択することが合理的とされる。

## 3. CAPM (Capital Asset Pricing Model)
個別資産の期待リターンを市場全体のリスクとの関係性で説明するモデル。
**公式**:

$$E[R_i] = R_f + \beta_i \, (E[R_m] - R_f)$$

- $E[R_i]$: 資産 $i$ の期待リターン
- $R_f$: リスクフリーレート
- $E[R_m]$: 市場ポートフォリオの期待リターン
- $\beta_i$: 市場に対する感応度
- $E[R_m] - R_f$: 市場リスクプレミアム

**ベータ $\beta$**: 市場全体の変動に対して、その資産がどれだけ動くかを示す指標。

$$\beta_i = \frac{\mathrm{Cov}(R_i, R_m)}{\mathrm{Var}(R_m)}$$

- **前提**: 投資家は平均・分散基準で行動し、同質的な期待を持ち、リスクフリーレートで自由に貸借できる市場均衡を仮定する。

## 4. シャープ・レシオ (Sharpe Ratio)
リスク1単位あたりに得られた超過リターンを示す。
- **計算式**:

$$\mathrm{SR} = \frac{E[R_p] - R_f}{\sigma_p}$$

  - $E[R_p]$: ポートフォリオの期待リターン、$R_f$: リスクフリーレート、$\sigma_p$: 超過リターン $R_p - R_f$ の標準偏差。期待リターンと標準偏差は同じ期間（通常は年率）で揃える。
- 値が高いほど、効率の良い運用ができていると判断される。
- **注意**: リターン分布が歪んでいる戦略、損失がまれに集中する戦略、自己相関が強い戦略では過大評価されやすい。最大ドローダウン、Sortino Ratio、Calmar Ratioも併用する。

## 5. ファクター・モデル
資産リターンを複数の要因（サイズ、バリュー、モメンタム等）で説明する手法。
- Fama-Frenchの3ファクター・モデルなどが有名。

## 6. 市場の期待変動幅 (Implied Move) の算出
オプション市場の価格から、市場参加者が織り込む満期までの変動幅を推定する実戦的な手法。

### ストラドル価格を用いた簡易推定
アット・ザ・マネー (ATM) のコールとプットの価格を合算することで、満期（SQ日）までの期待変動幅を求める。
**計算式**:

$$\text{Implied Move} = C_{\text{ATM}} + P_{\text{ATM}}$$

- **統計的意味**: ストラドル価格は「市場が支払っている絶対値変動の価格」であり、厳密な $1\sigma$ そのものではない。平均0の正規分布を仮定すると $E[|X|] = \sqrt{2/\pi}\,\sigma \approx 0.798\,\sigma$ なので、1標準偏差に換算するなら概算で $\sigma \approx \text{Straddle} / 0.798$ と見る。
- **実務上の解釈**: 現在価格 ± ストラドル価格は損益分岐点の目安であり、確率68.3%の範囲と断定しない。IV、スキュー、金利、配当、残存日数、板の厚さを合わせて確認する。

### 1日あたりの期待変動幅
年率のインプライド・ボラティリティ (IV) を用いて、1営業日あたりの変動目安を算出する（営業日換算で年252日と仮定）。
**計算式**:

$$\sigma_{1d} = \frac{\mathrm{IV}}{\sqrt{252}}$$

**DTE (Days to Expiration) を用いた標準化**:

$$\sigma_{\text{to expiry}} \approx \mathrm{IV} \cdot \sqrt{\frac{\mathrm{DTE}}{252}}$$

$$\sigma_{1d}^{\text{straddle}} \approx \frac{\sigma_{\text{straddle}} / 0.798}{\sqrt{\mathrm{DTE}}}$$

ここで $\sigma_{\text{straddle}}$ はATMストラドル価格（満期までの期待絶対値変動）を指す。

### IV MoveによるSQレンジ推定
IV Moveは、対象限月のIVからSQまでの想定レンジを推定する方法。プット・コール・パリティとは別物で、パリティはCall/Put/先物/権利行使価格の無裁定関係、IV Moveは満期までの変動幅の推定に使う。

**ATM IVを使う方法**:

$$\text{Move}_{1\sigma} = F \cdot \mathrm{IV}_{\text{ATM}} \cdot \sqrt{\frac{\mathrm{DTE}}{252}}$$

- **Call/Put IVを分ける方法**: 上方向はATMまたはOTM Call IV、下方向はATMまたはOTM Put IVを使う。Skewが強い場合、上下対称レンジより実態に近い。
- **$1\sigma$ レンジ**: 下限 $\approx F - \text{Move}_{\text{put},\,1\sigma}$、上限 $\approx F + \text{Move}_{\text{call},\,1\sigma}$
- **$2\sigma$ レンジ**: 下限 $\approx F - 2 \cdot \text{Move}_{\text{put},\,1\sigma}$、上限 $\approx F + 2 \cdot \text{Move}_{\text{call},\,1\sigma}$
- **日数の扱い**: 年率IVを使う場合、暦日なら365、営業日なら252で年率換算を統一する。日本市場では休場日とSQ日までの実残存時間を明記する。
- **注意**: IVはリスク中立分布上の価格であり、実現確率そのものではない。Smile/Skew、イベントIV、流動性、金利、配当、先物価格のズレを確認する。

### リスク管理への応用
- **ボラティリティ・スキュー**: プットとコールの価格差を見ることで、市場が上下どちらの「突き抜け（ファットテイル）」をより警戒しているかを判断する。
- **損益分岐点の把握**: ストラドル価格を超える動きは、オプション買いが利益化しやすい領域に入ったことを示す。ただし「異常事態」とは限らず、イベント前後のIV低下や流動性も含めて評価する。
